...
class EntradaController
{
    ...
    static async indexGet(req, res) {
        let data = await EntradaModel.consultar();
        res.send(data);
    }
    ...
    static async indexPost(req, res) {
        try {
            ...
            const insertedId = await EntradaModel.insertar(newData);

            res.status(201)
                .header('Location', `/entradas/${insertedId}`)
    ...
    static async itemGet(req, res) {
        ...
        let data = await EntradaModel.consultarPorId(id);
        if (data.length == 0) {
